...
function Button({refreshBalance}) {
  ...
  const transfer = async () => {
    ...
    const tx = await contract.methods.transfer(inputAddress, value).send({ from: accounts[0] });
    refreshBalance();

    console.log("vous avez envoyé ", tx.events.Transfer.returnValues.value, "à ", tx.events.Transfer.returnValues.to );
